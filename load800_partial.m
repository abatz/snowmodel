function [data,lon,lat,elev]=load800(var,year,y,x);

m=matfile('/home/abatz/sradratio800.mat');
sradratio=m.sradratio(y,x,:);
el2=matfile('/data/obs/obs/gridded/metdata/dem/metdata_elev');
el2=el2.elev;
...
clear tmax
data=tmaxdata+deltatmax(y,x,:)-273.15;
end
...
clear tmin
data=tmindata+deltatmin(y,x,:)-273.15;
end
...
data=tdmeandata+deltatdew(y,x,:)-273.15;
end
...
case 8,
m=matfile(['pet_800mPRISM_',num2str(year),'_CONUS.mat']);
data=m.PET(y,x,:);

case 9,
m=matfile(['SWE_',num2str(year),'.mat']);
data=m.SnowMelt(y,x,:);
end
data=single(data);
