function [SnowMelt,SnowWaterEq,SnowDepth, Sublimation]=runsnowmelt(cloudiness,tmax,tmin,precip_mm,solar,tdmean,vs,elev,startingSnow,startingSnowDepth_m);	
	...
	lambdaV = 2500;%				# (kJ/kg) latent heat of vaporization (this is used if SnowTemp >=0)
    lambdaS = 2850;%             # (kJ/kg) latent heat of sublimation (this used if SnowTemp<0)
	SnowHeatCap = 2.1;%			# kJ/kg/C
	LatHeatFreez = 333.3;%		# kJ/kg; note that LatentHeatFreeze * WaterDens = 
...
%#  New Variables	:
    ...
    rhoa 		= calcsatvap(tdmean);%		#	vapor density of atmoshpere (kg/m3) john's modification, we can use actual dewpoint temperature, rather than assume dew=tmin
...
%##	Energy Terms
    ...
    LE 		=zeros(size(precip_mm));%	#	Sensible Heat exchanged (kJ/m2/d)
...
%    f=find(startingSnowDepth_m == 0 & NewSnow==0);
    ...
    % sublimation added at end 
        ...
        [Sublimation(1),SnowWaterEq(1),SnowDepth(1)]=est_sub(EV,SnowWaterEq(1),SnowDepth(1),SnowDensity(1));
...
for i=2:length(vs)
    ...
    if SnowTemp(i)<0 E(i)=EV;end
        ...
        Energy(i)=Energy(i)*30; % scale for 30 days in a month
		...
		SnowWaterEq(i) = max(0,SnowWaterEq(i-1)-SnowMelt(i)+NewSnowWatEq(i));%
            ...
            % sublimation added at end 
        [Sublimation(i),SnowWaterEq(i),SnowDepth(i)]=est_sub(EV,SnowWaterEq(i),SnowDepth(i),SnowDensity(i));
        
end
SnowWaterEq=single(SnowWaterEq);SnowMelt=single(SnowMelt);
SnowDepth=SnowDepth(12);    

function [Sublimation,SnowWaterEq,SnowDepth]=est_sub(EV, SnowWaterEq,SnowDepth,SnowDensity);
    WaterDens =1000;%			# kg/m3
    lambdaS = 2850;%             # (kJ/kg) latent heat of sublimation (this used if SnowTemp<0)

    % sublimation added at end 
    Sublimation=-EV/lambdaS/WaterDens;
    Sublimation=Sublimation*30;
    ...
    if SnowWaterEq>Sublimation 
        ...
        SnowDepth=SnowWaterEq/SnowDensity;
        else
            Sublimation=0;SnowWaterEq=0;SnowDepth=0;
        end
    else
        if SnowWaterEq>0
            Sublimation=SnowWaterEq;
            SnowWaterEq=0;SnowDepth=0;
        else
            Sublimation=0;SnowWaterEq=0;SnowDepth=0;
        end
    end
